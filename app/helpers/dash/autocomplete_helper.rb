module Dash::AutocompleteHelper
  ...
  def autocomplete(options={})
    ...
    content_tag(:div, class: "input string ia-wrapper") do
      ...
      text_field_tag(:search, options[:value], placeholder: options[:placeholder], data: {url: options[:url], target: options[:target]}, id: id, autocomplete: "false", class: "form-control input-autocomplete") +
